...
struct GpsData
{
  ...
  //Quality of the satellite fix. 0 = invalid >0 are different types of valid (see nmea183 standard)
...
public:
  ...
  //{{{
  {
	//structs and GpsData are allocated in the freeRTOS heap 
    ...
    void* dataMem = pvPortMalloc(sizeof(GpsData));
    if (dataMem != nullptr)
    {
      gpsData_ = new (dataMem) GpsData{};
    }
  }
  //}}}
 ...
 * and returns ESP_FAIL or ESP_OK depending on success of the initialisation*/
  ...
  /*fill_data() reads the uart data, parses them and fills the GpsData struct with 
