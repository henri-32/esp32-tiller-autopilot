...
uint16_t NavigationSensors::compass_sensor_fusion()
{
  m_compass.read_raw();
}
...
NavigationSensors::NavigationSnapshot NavigationSensors::createSnapshot()
{
  ...
  snapshot.compass_hdg_dg.value = compass_sensor_fusion();
  snapshot.gps_cog_dg.value = m_gps.readCOG();
  snapshot.gps_sog_kts.value = m_gps.readSOG();
  snapshot.wind_angle_dg.value = m_wind.read();
  snapshot.stw_kts.value = m_nmea183Bus.readSTW();
